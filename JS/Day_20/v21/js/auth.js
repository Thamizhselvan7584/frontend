...
function togglePass(id,btn){const x=document.getElementById(id);if(!x)return;x.type=x.type==='password'?'text':'password';btn.textContent=x.type==='password'?'Show':'Hide'}
...
function setupAuth(){
 ...
 form.addEventListener('submit',e=>{e.preventDefault();const mode=form.dataset.mode;const msg=document.querySelector('#msg');
 if(mode==='register'){
